...
InputData <- read.xlsx(".\\Data\\Manuscript_2D_Simulation_Time\\Time_Memory.xlsx", sheet = 1)
...
Fig1 <- ggplot(InputData_Long, aes(x=NGenes, y=Value, group = Methods, color = Methods)) +
  ...
  scale_color_manual(values=Col_plate[1:5], drop = FALSE)
...
InputData <- read.xlsx(".\\Data\\Manuscript_2D_Simulation_Time\\Time_Memory.xlsx", sheet = 2)
colnames(InputData)[1] <- "Var"
InputData_Long <- melt(InputData, id = "Var")
InputData_Long$variable <- factor(InputData_Long$variable, levels = c("BSP",
                                                                      "spark",
...
InputData <- read.xlsx(".\\Data\\Manuscript_2D_Simulation_Time\\Time_Memory.xlsx", sheet = 3)
colnames(InputData)[1] <- "Var"
InputData_Long <- melt(InputData, id = "Var")
InputData_Long$variable <- factor(InputData_Long$variable, levels = c("BSP",
                                                                      "spark",
...
InputData <- read.xlsx(".\\Data\\Manuscript_2D_Simulation_Time\\Time_Memory.xlsx", sheet = 4)
colnames(InputData)[1] <- "Var"
InputData_Long <- melt(InputData, id = "Var")
InputData_Long$variable <- factor(InputData_Long$variable, levels = c("BSP",
                                                                      "spark",
...
# Print Figures -----------------------------------------------------------
# Figure 2C,2D
png(file=paste0(".\\Outputs\\Manuscript_2D_Simulation_Time.png"),
    width=9, height=4, units = "in", res = 600)
ggarrange(Fig1, Fig2, ncol=2, nrow=1, common.legend = TRUE, legend = "bottom")
dev.off()

# Supplementary Figure 3
png(file=paste0(".\\Outputs\\Manuscript_2D_Simulation_Memory.png"),
    width=9, height=4, units = "in", res = 600)
