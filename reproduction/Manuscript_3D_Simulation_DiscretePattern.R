...
Createcolorbar <- function(InputValues){
  ...
  Cont_part2 <- ramp.col(n = floor(30 / (max(median(InputValues) - min(InputValues), 1)) * (max(InputValues) - min(InputValues))) - 100, c(rgb(1,0,1), rgb(1,0,1)), alpha = 1.0)
...
htmlwidgets::saveWidget(as_widget(Fig1), "..\\Outputs\\Manuscript_3D_AdlSim_1.html")
